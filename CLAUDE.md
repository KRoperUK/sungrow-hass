...
| File | Responsibility |
| --- | --- |
| `sensor.py` | Builds `SungrowSensor` (plant), `SungrowDeviceSensor` (per-device) and `SungrowPlantDetailSensor` (plant-level health/tariffs from getPowerStationDetail, #178) entities from the stored coordinators. Classification comes from `measure_points.resolve_classification()`, so the Energy dashboard works; `_DIAGNOSTIC_CODES` marks the diagnostic points. |
...
| `schedule.py` | Repeating forced charge/discharge windows (#359/#433): evaluates the active window at startup, arms per-boundary callbacks, re-applies an enclosing window's mode when a nested one ends, and cancels in-flight boundary tasks on unload. Windows take an optional **weekday mask**, and a wrap-over-midnight window belongs to the day it *starts* on (`ScheduleWindow.contains()` takes a datetime for exactly that reason). Because `async_track_time_change` can only be armed per time-of-day, both boundary callbacks re-check containment before acting — otherwise a Monday-only window would actuate every Tuesday and release the battery on every non-matching day. |
