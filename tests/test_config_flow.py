...
async def _reauth_to_manual(hass: HomeAssistant, entry: MockConfigEntry) -> str:
    ...
    with patch("custom_components.sungrow._config_flow.cloud_oauth.asyncio.wait_for", side_effect=_timeout):
...
async def test_reauth_library_missing(hass: HomeAssistant):
    ...
    entry = _hub_entry(hass)
    with patch("custom_components.sungrow._config_flow._base.Auth", None):
        result = await entry.start_reauth_flow(hass)
...
async def test_late_callback_completes_from_manual_step(hass: HomeAssistant, mock_auth):
    ...
    with patch("custom_components.sungrow._config_flow.cloud_oauth.asyncio.wait_for", side_effect=_wait_for):
        result = await entry.start_reauth_flow(hass)
        assert result["type"] == data_entry_flow.FlowResultType.SHOW_PROGRESS
        await hass.async_block_till_done()
        await hass.async_block_till_done()
