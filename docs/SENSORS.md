...
**Energy dashboard classification (#431).** Genuinely-monotonic *lifetime* energy counters — battery total charge/discharge (`58606`/`58607`, `13034`/`13035`), grid import/export totals (meter `8030`/`8031`, `13125`/`13148`/`13175`/`83123`), ESS totals (`24622`/`24623`) and plant lifetime yield (`13134`), plus the local-Modbus `total_*` energy registers — are classified `device_class: energy`, `state_class: total_increasing`, kWh, so the **Energy dashboard** accepts them on both the cloud and local Modbus transports. *Daily/resetting* registers are treated more carefully: cloud daily points (e.g. `feed_in_energy_today`, `energy_purchased_today`) are reset server-side by iSolarCloud and stay `total_increasing`, but the **local-Modbus daily energy registers** (`daily_imported_energy`, `daily_exported_energy`, `daily_battery_charge`, `daily_battery_discharge`, `daily_exported_energy_from_pv`, `daily_battery_charge_from_pv`, `daily_direct_energy_consumption`, `daily_pv_gen_battery_discharge`) are firmware-dependent and can't be trusted to be monotonic — some SG-RS firmware never resets the daily register at midnight ([#400](https://github.com/KRoperUK/sungrow-hass/issues/400)) and the SH daily-imported register can read a flat 0 ([#401](https://github.com/KRoperUK/sungrow-hass/issues/401)) — so they are classified as a plain numeric **measurement** (no energy device class) and are deliberately kept out of the Energy dashboard to avoid corrupting long-term statistics. Local `daily_yield` is the exception: it's corrected by the derived-daily logic (see [Daily Solar Yield](#common-dashboard-values)), so it resets correctly and stays `total_increasing`. `daily_imported_energy` and `daily_exported_energy` work the same way: while a lifetime counter is available the integration derives them itself from `total_imported_energy` / `total_exported_energy` ([#471](https://github.com/KRoperUK/sungrow-hass/issues/471)) and publishes them as `device_class: energy`, `state_class: total_increasing` — a value we computed is monotonic within the day and resets at local midnight, so the Energy dashboard can take them as grid consumption / return to grid. The *raw* registers keep the plain-measurement treatment above, since a given firmware can't be trusted to be monotonic.
...
- **Battery / ESS** (hybrid systems): battery level (SOC), state of health, voltage, current, temperature, and total charge/discharge energy, plus cell/module health — **max/min cell voltage** and **max/min module temperature**, the derived **cell imbalance** (`max − min`, mV) and **module temperature spread** (`max − min`, °C) computed from them, operation status, DC-contactor status and fault-module ID. The derived spreads are the ones to alert on: a widening cell-voltage spread is the usual early sign of a weak cell, and a widening temperature spread points at a thermal problem, neither of which reads off the raw max/min pair. Health-oriented points (voltage, current, temperature, SOH, and all the cell/module health points) are marked *Diagnostic*; SOC and charge/discharge energy stay primary sensors for dashboards.
