...
- **Guided repairs** — surfaces iSolarCloud whitelist (E918/E919) and rate-limit (E998/E999) rejections as actionable Home Assistant **Repairs** with fix instructions, and **proactively warns** as the observed API call rate approaches the ~2000 calls/hour budget — naming the call type that dominates — so you can widen the interval *before* requests start being rejected.
...
| Field | Description |
|---|---|
| **Gateway** | Your region: **Europe**, **International**, **China**, **Australia**, or **India** |
...
- **Rate limiting.** If iSolarCloud rejects a poll for exceeding the call quota (E998/E999), the integration automatically **backs off** — doubling the effective interval up to a 1-hour cap — and raises a Home Assistant **Repair** suggesting a higher polling interval. It returns to your configured interval once the quota recovers. Separately, the integration tracks the request rate it generates itself (tagged by call type) and raises a **proactive** Repair once the observed rate approaches ~80% of the ~2000 calls/hour budget, *before* iSolarCloud rejects anything; the per-call-type rate is included in the diagnostics download (`api_call_rate`).
